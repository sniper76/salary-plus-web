...
import 'package:act_cms/domain/model/enum/terms_agreement_type.dart';
...
class TermsAgreementResult {
  final bool isAgreeReceiveMail;

  TermsAgreementResult({
    required this.isAgreeReceiveMail,
  });
}

class TermsAgreementBottomSheet extends StatefulWidget {
  const TermsAgreementBottomSheet({Key? key}) : super(key: key);

  static Future<TermsAgreementResult?> show(BuildContext context) {
    return showModalBottomSheet<TermsAgreementResult>(
      context: context,
...
class _TermsAgreementBottomSheetState extends State<TermsAgreementBottomSheet> {
  ...
  List<TermsAgreementType> _agreements = TermsAgreementType.values;
  ...
  void toggleIndividual(TermsAgreementType agreement) {
    setState(() {
  ...
  Widget build(BuildContext context) {
    return DragHandleBottomSheetLayout(
      ...
      children: [
        ...
        ..._agreements.map(
          (agreement) => BottomSheetCheckedItem(
            ...
            onPressed: _isRequiredAgreed
                ? () {
                    Navigator.of(context).pop(
                      TermsAgreementResult(
                        isAgreeReceiveMail: _checkedAgreementMap[TermsAgreementType.mail] ?? false,
                      ),
                    );
                  }
                : null,
