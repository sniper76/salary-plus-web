...
part 'post_stock_event.dart';
part 'post_stock_state.dart';
...
class PostBloc extends Bloc<PostEvent, PostState> {
  ...
  PostBloc() : super(const PostState()) {
    ...
    on<PostOnFetchPost>((event, emit) async {
      ...
      String searchKeyword = event.searchKeyword ?? state.searchKeyword;

      if (state.searchType == BoardSearchType.stockcode) {
        if (event.searchKeyword == "") {
          emit(state.copyWith(crearSimpleStock: true));
        } else {
          searchKeyword = state.selectedSimpleStock?.code ?? "";
        }
      }
      ...
      final res = await _fetchPost(
        ...
        postStatusType: state.postStatusType,
      ...
      if (res.isSuccess()) {
        ...
        emit(state.copyWith(isLoading: false, postList: users?.data, paging: users?.paging));
      } else {
        emit(state.copyWith(
          isLoading: false,
    ...
    on<PostOnUpdatedPost>((event, emit) async {});
  }
  ...
  Future<Result<DataResponse<List<Post>>, Exception>> _fetchPost({
    ...
    return _getPosts(
      ...
      searchKeyword: searchKeyword,
      status: postStatusType ?? state.postStatusType,
      page: page ?? 1,
  ...
  FutureOr<void> onPostOnInit(event, emit) async {
    try {
      ...
      final res = await _fetchPost(boardGroup: state.boardGroup, page: 1, size: apiLoadSize);
      if (res.isSuccess()) {
        final resData = res.tryGetSuccess();
        emit(state.copyWith(
            isLoading: false,
            boardCategoryList: boardGroupCategoryList,
            boardCategory: boardGroupCategoryList.isNotEmpty ? BoardGroupCategory.defaultCategory() : null,
