...
class ToolbarWidgetState extends State<ToolbarWidget> {
  ...
  void updateToolbar(Map<String, dynamic> json) {
    ...
    if (['decimal', 'lower-alpha', 'upper-alpha', 'lower-roman', 'upper-roman', 'disc', 'circle', 'square']
        .contains(listType)) {
      setState(mounted, this.setState, () {
          ...
          child: Opacity(
            ...
            child: Container(
              constraints: BoxConstraints(
                maxHeight:
                    _isExpanded ? MediaQuery.of(context).size.height : widget.htmlToolbarOptions.toolbarItemHeight + 15,
              ),
              child: _isExpanded
                  ? Padding(
                      padding: const EdgeInsets.all(5.0),
                      ...
                      child: CustomScrollView(
                        ...
                        slivers: [
                          SliverPersistentHeader(
                            pinned: true,
                            delegate:
                                ExpandIconDelegate(widget.htmlToolbarOptions.toolbarItemHeight, _isExpanded, () async {
                              setState(mounted, this.setState, () {
                                _isExpanded = !_isExpanded;
                              });
                              await Future.delayed(Duration(milliseconds: 100));
            ...
            child: CustomDropdownButton<String>(
              ...
              items: [
                ...
                CustomDropdownMenuItem(
                  value: 'h1',
                  child: PointerInterceptor(
                      child: Text('Header 1', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 32))),
                ),
                CustomDropdownMenuItem(
                  value: 'h2',
                  child: PointerInterceptor(
                      child: Text('Header 2', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 24))),
                ),
                CustomDropdownMenuItem(
                  value: 'h3',
                  child: PointerInterceptor(
                      child: Text('Header 3', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18))),
                ),
                CustomDropdownMenuItem(
                  value: 'h4',
                  child: PointerInterceptor(
                      child: Text('Header 4', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16))),
                ),
                CustomDropdownMenuItem(
                  value: 'h5',
                  child: PointerInterceptor(
                      child: Text('Header 5', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13))),
                ),
                CustomDropdownMenuItem(
                  value: 'h6',
                  child: PointerInterceptor(
                      child: Text('Header 6', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 11))),
                ),
              ],
            ...
            child: CustomDropdownButtonHideUnderline(
              child: CustomDropdownButton<double>(
                ...
                items: [
                  CustomDropdownMenuItem(
                    value: 1,
                    child: PointerInterceptor(child: Text('small')),
                  ),
                  CustomDropdownMenuItem(
                    value: 3,
                    child: PointerInterceptor(child: Text('medium')),
                  ),
                  CustomDropdownMenuItem(
                    value: 5,
                    child: PointerInterceptor(child: Text('large')),
                  ),
                  CustomDropdownMenuItem(
                    value: 6,
                    child: PointerInterceptor(child: Text('very large')),
                  ),
                  CustomDropdownMenuItem(
                    value: 7,
                    child: PointerInterceptor(child: Text('huge')),
                  ),
                ],
          ...
          toolbarChildren.add(ToggleButtons(
            ...
            onPressed: (int index) async {
              ...
              if (t.getIcons1()[index].icon == Icons.format_clear) {
                var proceed =
                    await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.clearFormatting, null, null) ??
                        true;
                if (proceed) {
                      ...
                      return PointerInterceptor(
                        child: AlertDialog(
                          ...
                          content: ColorPicker(
                            ...
                            title: Text('Choose a Color', style: Theme.of(context).textTheme.headline6),
                            width: 40,
                            height: 40,
                            spacing: 0,
                            runSpacing: 0,
                            ...
                            TextButton(
                              onPressed: () {
                                if (t.getIcons()[index].icon == Icons.format_color_text) {
                                  widget.controller.execCommand('foreColor',
                                      argument:
                                          (newColor.value & 0xFFFFFF).toRadixString(16).padLeft(6, '0').toUpperCase());
                                  setState(mounted, this.setState, () {
          ...
          toolbarChildren.add(ToggleButtons(
            ...
            onPressed: (int index) async {
              if (t.getIcons2()[index].icon == Icons.format_indent_increase) {
                var proceed =
                    await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.increaseIndent, null, null) ??
                        true;
                if (proceed) {
              ...
              if (t.getIcons2()[index].icon == Icons.format_indent_decrease) {
                var proceed =
                    await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.decreaseIndent, null, null) ??
                        true;
                if (proceed) {
                    ...
                    if (proceed) {
                      if (kIsWeb) {
                        ...
                        await widget.controller.editorController!
                            .evaluateJavascript(source: "\$('#summernote-2').summernote('lineHeight', '$changed');");
                      }
                      updateSelectedItem(changed);
                    }
                  }
        ...
        if (t.textDirection) {
          toolbarChildren.add(ToggleButtons(
            ...
            onPressed: (int index) async {
              ...
              var proceed = await widget.htmlToolbarOptions.onButtonPressed
                      ?.call(index == 0 ? ButtonType.ltr : ButtonType.rtl, _alignSelected[index], updateStatus) ??
                  true;
              if (proceed) {
                if (kIsWeb) {
        ...
        toolbarChildren.add(ToggleButtons(
          ...
          onPressed: (int index) async {
            ...
            if (t.getIcons()[index].icon == Icons.image_outlined) {
              var proceed =
                  await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.picture, null, null) ?? true;
              if (proceed) {
            ...
            if (t.getIcons()[index].icon == Icons.attach_file) {
              var proceed =
                  await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.otherFile, null, null) ?? true;
              if (proceed) {
                final filename = TextEditingController();
                final url = TextEditingController();
                final urlFocus = FocusNode();
          ...
          toolbarChildren.add(ToggleButtons(
            ...
            onPressed: (int index) async {
              ...
              if (t.getIcons1()[index].icon == Icons.undo) {
                var proceed =
                    await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.undo, null, null) ?? true;
                if (proceed) {
              ...
              if (t.getIcons1()[index].icon == Icons.redo) {
                var proceed =
                    await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.redo, null, null) ?? true;
                if (proceed) {
              ...
              if (t.getIcons1()[index].icon == Icons.help_outline) {
                var proceed =
                    await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.help, null, null) ?? true;
                if (proceed) {
                  await showDialog(
                      context: context,
                      builder: (BuildContext context) {
          ...
          toolbarChildren.add(ToggleButtons(
            ...
            onPressed: (int index) async {
              if (t.getIcons2()[index].icon == Icons.copy) {
                var proceed =
                    await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.copy, null, null) ?? true;
                if (proceed) {
              ...
              if (t.getIcons2()[index].icon == Icons.paste) {
                var proceed =
                    await widget.htmlToolbarOptions.onButtonPressed?.call(ButtonType.paste, null, null) ?? true;
                if (proceed) {
